      ...
      try {
        ...
        if (type === 'user_journey_map') {
          if (!item.journey_data) {
            throw new Error('Journey map has no data');
          }
          
          // Validate journey data structure
          const journeyData = typeof item.journey_data === 'string' 
          ...
          if (!item.content) {
            throw new Error('Mind map has no content');
          }
          exportData = {
            exportType: 'mind_map',
          ...
          if (!item.structure) {
            throw new Error('Architecture has no structure');
          }
          exportData = {
